        ...
        <div class="card-body">
            <h6 class="card-category">
                <a href="{{ route('blog.category', $article->category->slug) }}"
                    class="text-dark">{{ $article->category->name }}</a>
            </h6>
            <h4 class="card-title">
            ...
            <p class="card-tags">
                @foreach ($article->tags as $tag)
                    <a href="{{ route('blog.tag', $tag->slug) }}"><span style="background-color: {{ $tag->color }};"
                            class="badge badge-pill">{{ $tag->name }}</span></a>
                @endforeach
            </p>
            ...
            <p class="author">
                ...
                </a> &#8226; {{ Carbon\Carbon::parse($article->publish_date)->format('M j, Y') }} &#8226; {{ $article->read_time }} {{ __('min read') }}
            </p>
        </div>
    </div>
</div>
