 ...
 * Usage:
 *   # Submit a thought
 *   bun think.ts --thought "analysis here" --thoughtNumber 1 --totalThoughts 5 --nextThoughtNeeded true
 *
 *   # Submit a revision
 *   bun think.ts --thought "revised" --thoughtNumber 3 --totalThoughts 5 --nextThoughtNeeded true --isRevision --revisesThought 1
 *
 *   # Submit a branch
 *   bun think.ts --thought "alt path" --thoughtNumber 4 --totalThoughts 7 --nextThoughtNeeded true --branchFromThought 2 --branchId alt-approach
 *
 *   # Custom state file (for multi-agent isolation)
 *   bun think.ts --stateFile /tmp/think-agent1.json --thought "..." --thoughtNumber 1 --totalThoughts 3 --nextThoughtNeeded true
 *
 *   # View current state
 *   bun think.ts --status
 *
...
// Resolve state file path — custom or default
const STATE_FILE = values.stateFile || join(__dirname, ".think_state.json");
