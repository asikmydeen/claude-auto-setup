...
When running as a subagent or in parallel with other agents, use `--stateFile` to isolate your state:

```bash
bun scripts/think.ts --stateFile /tmp/think-$(echo $AGENT_ID | head -c 8).json --reset
```

This prevents parallel agents from clobbering each other's thinking state.
...
```bash
cd ~/.claude/skills/sequential-thinking && bun scripts/think.ts --status
```
...
| `--stateFile` | string | no | Custom state file path (for multi-agent isolation) |
...
- State is persisted to `scripts/.think_state.json` (or custom `--stateFile`) between invocations
