...
description: Regression-safe cleanup of AI-generated code with deletion-first workflow
---
...
- Check `codebase-patterns.md` for existing patterns before consolidating.
...
- Good fit: `ai-slop-cleaner src/server/routes/claude.ts src/server/lib/shared.ts`
- Good fit: a session handing off only the files changed in that session
...
- **Reviewer pass**: use our `code-reviewer` agent for the reviewer pass — inspect the cleanup plan, changed files, and verification evidence.
...
1. **Protect current behavior first**
   ...
   - Use our `test-writer` agent for regression test generation before editing.
