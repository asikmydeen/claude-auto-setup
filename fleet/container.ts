...
export class ContainerManager {
  ...
  // Fleet-specific settings.json path (stripped hooks/MCP — generated by prepareFleetConfig)
  ...
  /**
   * Generate a fleet-specific settings.json that strips host-only config.
   * Removes hooks (reference host scripts/services), MCP servers (hardcoded host paths),
   * and model override (fleet workers use Claude Code default for cost efficiency).
   * Must be called before run() or warmUp() for the settings to take effect.
   */
  prepareFleetConfig(outputDir: string): void {
    ...
    try {
      ...
      // Strip hooks — they reference host scripts/services unreachable from containers
      delete settings.hooks;

      // Strip MCP servers — paths are hardcoded to host filesystem
      delete settings.mcpServers;

      // Remove model override — fleet workers use Claude Code default (Sonnet)
      // instead of inheriting expensive Opus from host settings
      ...
      // Fall back to host settings.json (mounted as-is)
    }
  }
  ...
  run(opts: {
    ...
    // Mount global config (read-only) — rules, commands, agents, plugins, skills, scripts
  ...
  private getConfigMounts(homeDir: string, fleetUser: string): Array<[string, string]> {
    ...
    // Mount standalone skills (pua, sequential-thinking) if they exist.
    // Superpowers skills are already available via the plugins/ mount.
    const skillsDir = join(claudeDir, "skills");
    if (existsSync(skillsDir)) {
      mounts.push([skillsDir, join(fleetUser, ".claude", "skills")]);
    } else {
      // Fallback: mount superpowers skills from plugin cache directly
    ...
    // Mount scripts (enforce.sh, hook-handler.sh — referenced by hooks in settings.json)
    const scriptsDir = join(claudeDir, "scripts");
    if (existsSync(scriptsDir)) {
      mounts.push([scriptsDir, join(fleetUser, ".claude", "scripts")]);
    }

    // Mount fleet-specific settings.json (stripped hooks/MCP to avoid container errors).
    // Falls back to host settings.json if fleet settings not prepared.
    if (this._fleetSettingsPath && existsSync(this._fleetSettingsPath)) {
      mounts.push([this._fleetSettingsPath, join(fleetUser, ".claude", "settings.json")]);
    } else {
